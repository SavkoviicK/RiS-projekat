...
public class IzvestajService {
    ...
    // === VETERINAR: medicinski izveštaj (po zapisu) ===
    public byte[] generisiMedicinskiIzvestaj(Long zapisId) throws Exception {
        try (InputStream in = getClass().getResourceAsStream("/reports/medicinski_izvestaj.jrxml");
             Connection conn = dataSource.getConnection()) {

            if (in == null) {
                throw new IllegalStateException("Nije pronađen JRXML /reports/medicinski_izvestaj.jrxml");
            }

            JasperReport report = JasperCompileManager.compileReport(in);

            Map<String, Object> params = new HashMap<>();
            params.put("ZAPIS_ID", zapisId); // koristi se u WHERE klauzuli .jrxml-a
