...
public class AuthController {
    ...
    // Jednostavna regex provera email-a 
    ...
    public ResponseEntity<?> registracija(@RequestBody RegistracijaRequest req) {
        ...
        k.setIme(req.ime().trim());
        k.setPrezime(req.prezime().trim());
        k.setEmail(email);
        // posle validacije lozinke encode je bezbedan
        ...
        try {
            ...
            // npr. unique constraint na email
            return ResponseEntity.status(409).body(Map.of("message","Email je zauzet"));
        }
