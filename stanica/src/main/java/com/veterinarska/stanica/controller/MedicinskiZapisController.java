...
import com.veterinarska.stanica.service.IzvestajService;
...
public class MedicinskiZapisController {
    ...
    // VETERINAR: ljubimci sa OBAVLJEN pregledom za koje moze da unese zapis (lista za padajuci)
    ...
    // VLASNIK: svoje OBAVLJENE preglede
    ...
    // Svi zapisi koje je uneo prijavljeni veterinar (za tabelu na webu)
    ...
    public ResponseEntity<?> mojiZapisiVeterinar(Authentication auth) {
        ...
        List<MedicinskiZapisDTO> out = zapisRepo.findAll().stream()
                .filter(z -> z.getVeterinar() != null && Objects.equals(z.getVeterinar().getId(), vet.getId()))
                .sorted((a,b) -> {
                    int cmp = b.getDatum().compareTo(a.getDatum()); // datum opadajuće
    ...
    // “Karton” konkretnog ljubimca – VIDI SVE ZAPISE tog ljubimca (bilo koji veterinar)
    @GetMapping("/karton/{ljubimacId}")
    public ResponseEntity<?> kartonLjubimcaZaVeterinara(@PathVariable Long ljubimacId, Authentication auth) {
        Korisnik vet = ulogovan(auth);
        if (vet == null) return ResponseEntity.status(401).body(singleMsg("Niste prijavljeni."));
        if (vet.getUloga() != Uloga.VETERINAR)
            return ResponseEntity.status(403).body(singleMsg("Samo veterinar može videti karton."));

        if (ljubimacRepo.findById(ljubimacId).isEmpty()) {
            return ResponseEntity.status(404).body(singleMsg("Ljubimac ne postoji."));
        }

        List<MedicinskiZapisDTO> out = zapisRepo.findAll().stream()
                .filter(z -> z.getLjubimac() != null && Objects.equals(z.getLjubimac().getId(), ljubimacId))
                .sorted((a,b) -> {
                    int cmp = b.getDatum().compareTo(a.getDatum());
                    return (cmp != 0) ? cmp : Long.compare(b.getId(), a.getId());
                })
                .map(AppMapper::toDTO)
                .toList();
    ...
    // ---------------- PDF (Jasper) – samo veterinar ----------------
    @GetMapping(value = "/pdf/{zapisId}", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<?> preuzmiPdf(@PathVariable Long zapisId, Authentication auth) {
        Korisnik korisnik = ulogovan(auth);
        if (korisnik == null) return ResponseEntity.status(401).body(singleMsg("Niste prijavljeni."));

        Optional<MedicinskiZapis> opt = zapisRepo.findById(zapisId);
        if (opt.isEmpty()) return ResponseEntity.status(404).body(singleMsg("Medicinski zapis ne postoji."));

        MedicinskiZapis zapis = opt.get();
        if (!mozePristupPdfu(korisnik, zapis)) {
            return ResponseEntity.status(403).body(singleMsg("Nemate pristup ovom izveštaju."));
        }

        try {
            byte[] pdf = izvestajService.generisiMedicinskiIzvestaj(zapisId); // Jasper
            String fileName = buildFileName(zapis);

            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + fileName + "\"; filename*=UTF-8''" + url(fileName))
                    .contentType(MediaType.APPLICATION_PDF)
                    .body(pdf);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().body(singleMsg("Greška pri generisanju PDF izveštaja."));
        }
    }
    ...
    private static boolean mozePristupPdfu(Korisnik korisnik, MedicinskiZapis zapis) {
        // bilo koji veterinar ima pristup svim medicinskim zapisima
        return korisnik.getUloga() == Uloga.VETERINAR;
    }

    private static String buildFileName(MedicinskiZapis z) {
        String lj = (z.getLjubimac() != null && z.getLjubimac().getIme() != null) ? z.getLjubimac().getIme() : "ljubimac";
        String id = z.getId() != null ? String.valueOf(z.getId()) : "n";
        return ("izvestaj_" + lj + "_" + id + ".pdf").replace(' ', '_');
    }
