...
<div class="wrap">
  ...
  <div id="zone" class="layout">
    <div class="card">
      <div class="tabs">
        <div id="tabChats" class="tab active">Razgovori</div>
        <div id="tabKontakti" class="tab">Prijatelji</div>
      </div>
      ...
      <section id="secChats">
        ...
        <ul id="ulChats" class="list">
          <li class="muted" id="chEmpty">Učitavam…</li>
        </ul>
      </section>
      ...
      <section id="secKontakti" style="display:none">
        ...
        <input id="inpSearch" class="input search" type="text" placeholder="Pretraga (ime, prezime ili email)…"/>

        <ul id="ulFriends" class="list"><li class="muted">Učitavam…</li></ul>
      </section>
    </div>
    ...
    <div class="card chatWrap">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        <div class="muted" id="listMsg">Učitavam…</div>
      </div>

      <div id="chatList" class="chatList"></div>

      <div class="composer">
        <textarea id="inpSadrzaj" rows="2" class="input" placeholder="Napiši poruku…" style="flex:1"></textarea>
        <button id="btnSend" class="btn">Pošalji</button>
...
async function fetchJSON(url, opts={}, msgId=null, okMsg=null){
  ...
  if(!res.ok){
    const txt = isJson ? (await res.json()).message || 'Greška' : (await res.text().catch(()=>('HTTP '+res.status)));
    if(msgId){ const el=document.getElementById(msgId); if(el) el.textContent='Greška: '+txt; }
    throw new Error(txt);
  }
...
if (tabChats) tabChats.onclick = ()=>setTab('ch');
if (tabKont)  tabKont.onclick  = ()=>{ setTab('kont'); loadFriendsIfNeeded(); };
...
async function loadFriendsIfNeeded(){
...
function extractRole(u){
  const r = u.uloga ?? u.role ?? u.rola ?? null;
  if (r && typeof r === 'string') return r.toUpperCase().replace(/^ROLE_/,'');
  if (r && r.name) return String(r.name).toUpperCase().replace(/^ROLE_/,'');
  const arr = u.roles || u.authorities || [];
  if (Array.isArray(arr) && arr.length){
    const first = arr[0];
    return String(first.authority || first).toUpperCase().replace(/^ROLE_/,'');
  }
  return '';
}
...
function normalizeUsers(arr){
  ...
  const out = (Array.isArray(arr)?arr:[]).map(u=>{
    ...
    const label = (niceFullName(ime, prez) || email || (id ? ('#'+id) : '')) + (role?` (${role})`:'');
    return { id, email, ime, prezime:prez, role, label };
  }).filter(u=>{
...
async function loadFriends(){
  const ulF = document.getElementById('ulFriends');
  if (ulF) ulF.innerHTML='<li class="muted">Učitavam…</li>';

  try{
    const fr = await fetchJSON('/api/prijatelji', { headers: authHeader() });
    FRIENDS = normalizeUsers(fr);
  }catch(_){ FRIENDS = []; }

  paintFriendList();
}

function paintFriendList(filter=''){
  const q = (filter||'').trim().toLowerCase();
  const ulF = document.getElementById('ulFriends');
  if(!ulF) return;

  ulF.innerHTML='';
  const view = q ? FRIENDS.filter(x => (x.label.toLowerCase().includes(q) || (x.email||'').toLowerCase().includes(q))) : FRIENDS;
  if(!view.length){ ulF.innerHTML='<li class="muted">Nema rezultata.</li>'; return; }
  view.forEach(x=>{
    const full = niceFullName(x.ime, x.prezime);
    const safeEmail = clean(x.email);
    const roleTag = x.role ? ` (${x.role})` : '';
    const whoText = (full ? `${full} – ${safeEmail}` : (safeEmail || `Korisnik #${x.id}`)) + roleTag;
    const visibleName = full || 'Korisnik';
    const li=document.createElement('li');
    li.innerHTML = `
      <div>
        <strong>${visibleName}</strong>
        <span class="muted">${whoText} (#${x.id})</span>
      </div>
      <button class="btn-tone">Pošalji poruku</button>`;
    li.querySelector('button').onclick = ()=>{
      setPeer(x.id);
      setSelectRecipient(x.id, whoText);
      setTab('ch');
    };
    ulF.appendChild(li);
  });
}
const inpSearchEl = document.getElementById('inpSearch');
if (inpSearchEl) {
  inpSearchEl.addEventListener('input', (e)=> paintFriendList(e.target.value));
}
...
async function fillRecipientSelect(){
  ...
  if (!sel) return;
  sel.disabled=true; sel.innerHTML='<option value="">Učitavam…</option>';
  ...
  const normalized = (users||[]).map(u=>{
    ...
    const email = u.email || u.username || u.korisnikEmail || u.mail || '';
    const ime = u.ime || '';
    const prezime = u.prezime || '';
    const role = extractRole(u);
    return { id, email, ime, prezime, role };
  }).filter(u=>{
    if(!u.id) return false;
    if(ME.id && u.id===ME.id) return false;
    if(seen.has(u.id)) return false; seen.add(u.id); return true;
  }).sort((a,b)=>
    String((a.ime||'')+(a.prezime||'')+(a.email||''))
      .localeCompare(String((b.ime||'')+(b.prezime||'')+(b.email||'')))
  );

  sel.innerHTML = '<option value="">— izaberi primaoca —</option>';
  normalized.forEach(u=>{
    const full = niceFullName(u.ime,u.prezime);
    const safeEmail = clean(u.email);
    const base = full ? `${full} – ${safeEmail}` : (safeEmail || `Korisnik #${u.id}`);
    const label = base + (u.role ? ` (${u.role})` : '');
  ...
  sel.onchange = ()=>{
    const v = sel.value && /^\d+$/.test(sel.value) ? Number(sel.value) : null;
    if (v){ setPeer(v); }
  };
}
...
function setSelectRecipient(id, labelText){
  ...
  if (![...sel.options].some(o=>o.value===String(id))){
    const opt=document.createElement('option');
    opt.value=String(id); opt.textContent=(labelText||('Korisnik #'+id))+` (#${id})`;
    sel.appendChild(opt);
  }
  sel.value=String(id);
  CURRENT_PEER_ID=id;
...
async function posaljiPoruku(){
  ...
  const txtEl = document.getElementById('inpSadrzaj');
  const sadrzaj = (txtEl && txtEl.value || '').trim();

  if(!recipId || !sadrzaj){
    const sm=document.getElementById('sendMsg');
    if (sm) sm.textContent = !recipId ? 'Izaberi korisnika.' : 'Unesi sadržaj poruke.';
    return;
  }
  if (btn) btn.disabled=true;
  const sm=document.getElementById('sendMsg');
  if (sm) sm.textContent='Šaljem…';
  try{
...
async function loadAll(){
  const listMsg = document.getElementById('listMsg');
  if (listMsg) listMsg.textContent='Učitavam…';
  try{
    ...
    if(info) info.textContent='Ne mogu da učitam poruke (API).';
  }
}
...
window.addEventListener('load', async ()=>{
  ...
  try{
    ...
    const hint = localStorage.getItem('peerHintId');
    if (hint && /^\d+$/.test(hint)) {
      setSelectRecipient(Number(hint));
      localStorage.removeItem('peerHintId');
    }
    const btnSend = document.getElementById('btnSend');
    if (btnSend) btnSend.addEventListener('click', posaljiPoruku);
    const btnSve = document.getElementById('btnSve');
    if (btnSve) btnSve.addEventListener('click', ()=>{
    ...
    console.error('Poruke init error:', e);
    if(zone) zone.style.display='grid';
    const info=document.getElementById('info');
    if(info) info.textContent='Greška pri učitavanju. Osvežite stranu (Ctrl+F5).';
  }
});
</script>
</body>
